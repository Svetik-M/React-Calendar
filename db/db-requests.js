...
module.exports = {
  ...
  findByUsername(username, password, cb, done, req) {
    process.nextTick(() => {
      let user;
  ...
  verifyUser(err, user, password, done) {
  ...
  createNewUser(err, user, password, done, req) {
    if (err) return done(err);
    if (user) return done(null, false);
  ...
  addEvent(req, res) {
    db.one(
      `INSERT INTO events(user_id, title, start_date, end_date, place, category, description,
  ...
  editEvent(req, res) {
    db.none(
      `UPDATE events SET  title=$3, start_date = $4, end_date = $5, place = $6, category = $7,
      description = $8, is_repeat = $9, repeat_rate = $10, repeat_duration = $11, repeat_end = $12
      WHERE id = $1 and user_id = $2`,
      [req.body.id, req.user.id, req.body.title, req.body.start_date, req.body.end_date,
        req.body.place, req.body.category, req.body.description, req.body.repeat,
